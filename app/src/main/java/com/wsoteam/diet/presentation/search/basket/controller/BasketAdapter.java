...
public class BasketAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  public BasketAdapter(List<ISearchResult> foods, Context context) {
    this.foods = foods;
  ...
  @Override public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
    switch (holder.getItemViewType()) {
      case HEADER_TYPE:
        ((BasketHeaderVH) holder).bind((HeaderObj) foods.get(position));
        break;
      case ITEM_TYPE:
        ((BasketItemVH) holder).bind((Result) foods.get(position));
        break;
    }
  }
